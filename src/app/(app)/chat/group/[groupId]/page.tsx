  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--off)' }}>
      ...
      <div style={{ padding: '10px 16px calc(env(safe-area-inset-bottom) + 10px)', background: 'white', borderTop: '1px solid var(--line)', display: 'flex', gap: 8, alignItems: 'center', flexShrink: 0 }}>
        <button onClick={() => setShowAttachMenu(!showAttachMenu)} style={{ width: 34, height: 34, borderRadius: '50%', background: 'var(--surf)', border: '1px solid var(--line)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0, fontSize: 16 }}>📎</button>
        ...
        <input ref={imageRef} type="file" accept="image/*" onChange={(e) => { const f = e.target.files?.[0]; if (!f) return; setSelectedImage(f); setImagePreview(URL.createObjectURL(f)); e.target.value = '' }} style={{ display: 'none' }} />
        <input ref={fileRef} type="file" accept=".pdf" onChange={(e) => { const f = e.target.files?.[0]; if (!f) return; setSelectedFile(f); e.target.value = '' }} style={{ display: 'none' }} />
      </div>
      ...
      {showAttachMenu && (
        <>
          ...
          <div style={{ position: 'fixed', bottom: 80, left: 16, background: 'white', borderRadius: 12, border: '1px solid var(--line)', padding: '8px', zIndex: 201, boxShadow: '0 4px 20px rgba(0,0,0,.12)' }}>
            <button onClick={() => { imageRef.current?.click(); setShowAttachMenu(false) }} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 16px', background: 'none', border: 'none', cursor: 'pointer', fontSize: 13, color: 'var(--txt)', borderRadius: 8, width: '100%' }}>📷 画像を添付</button>
            <button onClick={() => { fileRef.current?.click(); setShowAttachMenu(false) }} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 16px', background: 'none', border: 'none', cursor: 'pointer', fontSize: 13, color: 'var(--txt)', borderRadius: 8, width: '100%' }}>📄 PDFを添付</button>
          </div>
        </>
      )}
    </div>
